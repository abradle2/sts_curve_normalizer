...
gain = 1E8;
smudge = 0.03;      %This is the factor that multiplies I/V to get rid of infinities
...
for i = 1:header_lines
    ...
    if ~isempty(deltaV_location)
        ...
        deltaV = sscanf(deltaV_string, '%g');  %Pull out the number from the sensitivity
...
dIdV = gain*sens*sqrt(2)/(deltaV*2.5)*LIY;

dIdV_normalized = LIY./(gain*I_t./V +smudge);
